...
print("<s> Transformation.")  # Mensaje apply_euclidean_transformation
...
def apply_similarity_transformation(image, angle, tx, ty, s):
    """
    Aplica una transformación de similaridad (rotación + traslación + escalado) a una imagen.
    ...
    # Calcula el tamaño del nuevo lienzo
    cos_angle = abs(s * np.cos(angle))
    sin_angle = abs(s * np.sin(angle))
    new_width = int(cols * cos_angle + rows * sin_angle)
    new_height = int(cols * sin_angle + rows * cos_angle)
    ...
    # Calcula la matriz de transformación de similaridad
    similarity_matrix = np.array([
        [s * np.cos(angle), -s * np.sin(angle), 0],
        [s * np.sin(angle),  s * np.cos(angle), 0]
    ])

    # Ajusta la traslación para centrar la imagen correctamente
    similarity_matrix[0, 2] = (new_width / 2) - (original_center[0] * similarity_matrix[0, 0] + original_center[1] * similarity_matrix[0, 1]) + tx
    similarity_matrix[1, 2] = (new_height / 2) - (original_center[0] * similarity_matrix[1, 0] + original_center[1] * similarity_matrix[1, 1]) + ty

    # Aplica la transformación a la imagen con el nuevo tamaño
    transformed_image = cv2.warpAffine(image, similarity_matrix, (new_width, new_height))

    return transformed_image

def get_transformation_parameters():
    """
    Solicita al usuario los valores de ángulo, traslación en x, traslación en y y escala.
    ...
    try:
        ...
        # Solicita el factor de escala
        s = float(input("Ingrese el factor de escala (por ejemplo, 1.0 para mantener el tamaño): "))

        return angle, tx, ty, s
    except ValueError:
...
while True:
    ...
    if k == ord('s'):  # Si se presiona 's', guarda la selección
        if ix != -1 and iy != -1 and fx != -1 and fy != -1:  # Verifica que las coordenadas sean válidas
            ...
            if cropped_img.size > 0:  # Verifica que la selección no esté vacía
                ...
                angle, tx, ty, s = get_transformation_parameters()
                cropped_img = apply_similarity_transformation(cropped_img, angle, tx, ty, s)
